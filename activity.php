...
$query="SELECT * from myevents ORDER BY date DESC LIMIT 500";
$result=mysqli_query($conn,$query) or die(mysqli_error());

echo "<table border='2'>";
echo "<tr><th align='center' strong style='font-size: 10px;'>일자</th>";
echo "<th align='center' strong style='font-size: 20px;'>구분</th>";
echo "<th align='center' strong style='font-size: 20px;'>내역</th>";
echo "<th align='center' strong style='font-size: 20px;'>지불</th>";
...
{
	if($row['type'] == 'DIARY') 
		echo "<tr style='background-color:#C0C0C0'><td align='center' strong style='font-size: 10px;'><b>".date('Y-m-d',strtotime($row['date']))."</b></td>";
	else
		echo "<tr><td align='center' strong style='font-size: 10px;'>".date('Y-m-d',strtotime($row['date']))."</td>";
	...
	echo "<td align=right strong style='font-size: 20px;'>".$row['paymethod']."</td>";
	if($row['price']>0)
	{
